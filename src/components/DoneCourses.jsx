...
const DoneCourses = () => {
      ...
      useEffect(() => {
        const fetchData = async () => {
            const {data} = await supabase.from("Courses").select().eq("user",userName).eq("percent",90)
            setCourses(data)
        }
        fetchData()
    ...
    return (
        <div>
            <div className='w-full overflow:hidden pb-4 pl-4 bg-white'>
                <h1 className='text-3xl font-bold p-4 mb-4'>Completed Courses</h1>
                    { courses && (
                    <div>
                        ...
                        <div key={index} className='flex w-[95%] mb-4 p-4 ml-2 bg-white drop-shadow-xl rounded-lg'>
                            <div className='mr-8 flex'>
                                ...
                                <button className='bg-orange-400 ml-12 p-2 px-6 drop-shadow-xl rounded-full hover:bg-orange-200'>
                                    <Link to="/quiz" state={{user:userName,title:course.title,id:course.id}}>Take Exam</Link>
                                </button> 
                            </div>
                        </div>
                        ))}
